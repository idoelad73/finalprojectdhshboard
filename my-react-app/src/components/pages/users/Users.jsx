      ...
      <table className="w-full">
        <thead>
          <tr className="border-b-2 border-slate-200">
            ...
            <th className="text-left py-4 px-6 text-sm font-bold text-slate-700 uppercase tracking-wider">Role</th>
            <th className="text-left py-4 px-6 text-sm font-bold text-slate-700 uppercase tracking-wider">Joined</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {users.map(u => (
            <tr key={u._id} className="hover:bg-slate-50 transition-colors">
              <td className="py-4 px-6 text-slate-900 font-medium">{u.user_name}</td>
              <td className="py-4 px-6 text-slate-600">{u.user_email}</td>
